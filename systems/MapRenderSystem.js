...
export class MapRenderSystem extends System {
    ...
    update(deltaTime) {
        ...
        if (lightingState && lightingState.isLit) {
            ...
            gradient.addColorStop(0.5, 'rgba(255, 180, 60, 0.10)');
        ...
        for (const entity of entities) {
            ...
            if (entity.hasComponent('LightSource')) {
                ...
                this.ctx.save();
                this.ctx.shadowColor = light.color || 'rgba(255,255,180,0.7)';
                this.ctx.shadowBlur = light.glow || 24;
            } else {
                this.ctx.save();
            }
        ...
        for (const entity of entities) {
            ...
            const pos = entity.getComponent('Position');
            const light = entity.getComponent('LightSource');
            const npcScreenX = (pos.x - startX) * this.SCALE_FACTOR + this.TILE_SIZE * this.SCALE_FACTOR / 2;
            const npcScreenY = (pos.y - startY) * this.SCALE_FACTOR + this.TILE_SIZE * this.SCALE_FACTOR / 2;
            const npcRadius = (light.radius || 3) * this.TILE_SIZE * this.SCALE_FACTOR;

            if (
                !Number.isFinite(playerScreenX) ||
                !Number.isFinite(playerScreenY) ||
                !Number.isFinite(pxRender)
            ) {
                console.error('Non-finite value for lighting gradient:', { playerScreenX, playerScreenY, pxRender });
                continue;
            } 
