    ...
    addLevel({ tier, customLevel = null, transitionDirection = null }) {
        ...
        if (!levelEntity) {
            ...
            if (tier === 0) {
                ...
                // ... (procedural level generation remains unchanged)
                const journeyBossLevels = [3, 6];
                let hasBossRoom = false;
...
export class LevelSystem extends System {
    ...
    init() {
        ...
        if (!this.entityManager.getEntitiesWith(['Tier']).some(e => e.getComponent('Tier').value === 0)) {
            ...
            this.entityManager.addComponentToEntity(levelEntity.id, new ExplorationComponent());
            this.entityManager.addComponentToEntity(levelEntity.id, new SpatialBucketsComponent());
            //  this.entityManager.addComponentToEntity(levelEntity.id, new MapComponent({ map: [], walls: [], floors: [] }));
        ...
        if (isNewGame) {
            ...
            // For loaded games, generate levels from 1 up to the current tier (if not already generated)
            for (let tier = 0; tier <= gameState.tier; tier++) {
                if (!this.entityManager.getEntitiesWith(['Tier']).some(e => e.getComponent('Tier').value === tier)) {
                    this.addLevel({ tier });
                }
            }
...
addLevel({ tier, customLevel = null, transitionDirection = null }) {
    ...
    this.isAddingLevel = true;
    console.log(`LevelSystem.js: addLevel - Starting for tier ${tier},customLEvel ${customLevel}, transitionDirection: ${transitionDirection}`);

    this.entityManager.setActiveTier(tier);
    console.log(`LevelSystem.js: addLevel - Set active tier to ${tier}, active tier: ${this.entityManager.getActiveTier()}`);
    let levelEntity = this.entityManager.getEntitiesWith(['Tier']).find(e => e.getComponent('Tier').value === tier);

    console.warn(`LevelSystem.js: addLevel - Found levelEntity for tier ${tier}:`, levelEntity ? levelEntity.id : 'None');
    ...
    if (!levelEntity) {
        ...
        if (tier === 0) {
            ...
            this.customLevelSystem.loadCustomLevel(tier, levelEntity).then(levelData => {
                ...
                // Populate EntityListComponent
                const entityListComp = levelEntity.getComponent('EntityList');
                entityListComp.walls = levelData.walls || [];
                entityListComp.floors = levelData.floors || [];
                entityListComp.stairs = levelData.stairs || [];
                entityListComp.portals = levelData.portals || [];
                entityListComp.monsters = levelData.monsters || [];
                entityListComp.treasures = levelData.treasures || [];
                entityListComp.fountains = levelData.fountains || [];
                entityListComp.rooms = levelData.roomEntityIds || [];
                entityListComp.npcs = levelData.npcs || [];

                // Adjust player position
                console.log(`LevelSystem.js: addLevel - Adjusting player position for tier ${tier}, transitionDirection: ${transitionDirection}`);
                this.adjustPlayerPosition(levelEntity, transitionDirection === 'down' ? levelData.stairsUp : (levelData.stairsDown || levelData.stairsUp));

                // Ensure room connections
                const ercPasses = 3;
                for (let i = 0; i < ercPasses; i++) {
                    this.ensureRoomConnections(levelEntity);
                }
            ...
            console.log(`LevelSystem.js: Generating procedural level for tier ${tier}`);
            const journeyBossLevels = [3, 6];
            let hasBossRoom = false;
            ...
            entityListComp.floors = levelData.floors;
            entityListComp.rooms = levelData.roomEntityIds;
            entityListComp.fountains = this.generateFountains(tier, levelData, entityListComp);

            // Place stairs
            this.placeStairs(levelEntity, levelData, hasBossRoom);
            entityListComp.treasures = this.generateLootEntities(tier, levelData.map, levelData.roomEntityIds);
            mapComp.stairsUp = levelData.stairsUp;
            mapComp.stairsDown = levelData.stairsDown;
        ...
        console.log(`LevelSystem.js: addLevel - Reusing existing level entity for tier ${tier}`);
        this.checkLevelAfterTransitions({ tier, levelEntity });
    }
...
isWalkable(x, y) {
   ...
   if (!gameState || !gameState.gameStarted || gameState.gameOver) {
       console.warn(`LevelSystem.js: checkLevelAfterTransitions - Game not started or already over, skipping checks for tier ${tier}`);
       return;
   }
   */
    if (gameState.isTransitioning) {
        //console.log(`LevelSystem.js: checkLevelAfterTransitions - Transition in progress, skipping checks for tier ${tier}`);
        return;
    }
    //console.log(`LevelSystem.js: checkLevelAfterTransitions - Starting for tier ${tier}`);
    const entity = levelEntity || this.entityManager.getEntitiesWith(['Tier']).find(e => e.getComponent('Tier').value === tier);
